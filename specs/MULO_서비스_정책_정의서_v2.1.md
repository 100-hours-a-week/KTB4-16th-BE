...
| `PLACE-016` | **미확정** | place 재사용 기준 | 동일·근접 좌표에서 기존 `place`를 재사용할지 새 `place`를 생성할지 기준은 아직 확정하지 않는다. | 추가 결정 필요 |
...
| `MUSIC-004` | **현재 설계** | 신규 음악 재검증 | 자물쇠 생성 시 신규 `externalTrackId`는 Spotify API로 재검증하고 필요하면 `music_tracks`에 저장한다. | API 명세 상세2차 정책 요약 R15 / R106 |
...
| `UPLOAD-006` | **현재 설계** | 업로드 포맷/용량 | JPG, JPEG, PNG, HEIC를 허용하며 최대 10MB이다. | API 명세 상세2차 R90-R96 |
